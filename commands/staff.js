...
exports.run = async (client, message, args) => {
    ...
    let msg = await message.channel.send(
        new Discord.MessageEmbed()
            ...
            .setDescription("Do you really want to make this user staff ?")
            .setColor("RANDOM")
    ...
    collector.on('collect', async (r)=>{
        switch (r.emoji.name)
        {
            case "✅":
                ...
                try{
                    message.mentions.members.first().roles.remove(client.config.greenRole);
                }catch{}
                try{
                    message.mentions.members.first().roles.remove(client.config.redRole);
                }catch{}
                try{
                    message.mentions.members.first().roles.add(client.config.staffRole);
                }catch{}
                msg.reactions.removeAll();
                collector.stop();
                msg.channel.send("Welcome to staff !");
                let users = JSON.parse(fs.readFileSync('./users.json'));
                    for (let u of users) {
                        if (u.discord == message.mentions.users.first().id)
                            u.team = "staff";
